...
class Tab1 extends Component {
  ...
  async componentDidMount() {
    ...
    await this.setStateAsync({
      publicKey,
      balances: balances ?? [0, 0, 0],
      usdConversion: usdConversion ?? [1, 1, 1],
      nfcSupported: nfcSupported && nfcEnabled,
    });
  ...
  async getBalances() {
    let balanceETH = await this.provider.getBalance(this.state.publicKey);
      ...
      EVM.tokens.slice(1).map(
        token =>
          new Promise(async resolve => {
            ...
            let balance = await contract.balanceOf(this.state.publicKey);
            balance = parseFloat(
  ...
  async getUSD() {
    try {
      ...
      console.log(res);
      this.setState({usdConversion: [res.ETH, res.USDC, res.USDT, res.WBNB]});
